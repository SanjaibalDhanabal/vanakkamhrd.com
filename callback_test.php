...
function generate_poster_pdf($bg_image, $user_photo, $name, $designation, $company, $output_pdf) {
    ...
    $pdf->Image($user_photo, 160, 800, 600, 600);
	...
	//var_dump($order_id);exit;
        if ($data) {
            ...
            
            {
                ...
                $poster_bg = "temp/poster_123.png"; // Your blank poster template
              ...
              echo $poster_pdf;exit;
                // Generate Invoice PDF (fill name, email, company, invoice number, ticket id)
                $invoice_file = "temp/invoice_".$data["id"].".pdf";
               // echo $invoice_url = "https://vanakkamhrd.com/invoice_template.php?name=".urlencode($data["name"])."&email=".urlencode($data["email"])."&company=".urlencode($data["company"])."&invoice_number=2025/".$invoice_number."&ticket_id=".$ticket_id;
               // exit;
              exec('wkhtmltopdf "'.$invoice_url.'" '.$invoice_file);
                ...
                $sent = send_email_with_attachments(
                    ...
                    "Welcome to Annual TN HR Summit 2025!",
                    $message,
                    [$poster_pdf, $invoice_file]
                );
                if(!$sent) {
                    var_dump("Failed to send email to ".$data["email"]);
                }
            }
        } else {
            var_dump("No record found for order_id: " . $order_id);
        }
